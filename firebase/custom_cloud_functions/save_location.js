...
exports.saveLocation = functions.region("europe-west3").auth.user().onCreate(async (user) => {
  ...
  try {
    ...
    const userMatches = await orgsRef.where("userEmailList", "array-contains", userEmail).get();
    ...
    // eslint-disable-next-line max-len
    const adminMatches = await orgsRef.where("adminEmailList", "array-contains", userEmail).get();
